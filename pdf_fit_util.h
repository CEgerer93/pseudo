...
struct pdfFitParams_t
{
  ...
  gsl_vector *t4_sigmaN, *t4_fitParams;
  int nParams;
  ...
  std::vector<double> t4_prior {0.0, 0.0, 0.0};
  std::vector<double> t4_width {0.25, 0.25, 0.25};
...
: pdfType(_pt), alpha(_a), beta(_b)
  {
    ...
    lt_fitParams = lt; az_fitParams = az; t4_fitParams = t4;
    lt_sigmaN = gsl_vector_alloc(lt->size);
    az_sigmaN = gsl_vector_alloc(az->size);
    t4_sigmaN = gsl_vector_alloc(t4->size);
